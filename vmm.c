...
int init_memory(int rvm_fd, int vmid, const char *bios_file) {
    // RAM
    struct rvm_guest_memory_region_add_args region = {vmid, 0, GUEST_RAM_SZ};
    char *ram_ptr = (char *)(intptr_t)ioctl(rvm_fd, RVM_GUEST_MEMORY_REGION_ADD, &region);
    ...
    char *bios_ptr = (char *)(intptr_t)ioctl(rvm_fd, RVM_GUEST_MEMORY_REGION_ADD, &region);
    printf("bios_ptr = %p\n", bios_ptr);
