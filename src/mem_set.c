...
void mem_set_init(struct vm_mem_set* mem_set) { mem_set->cnt = 0; }
...
uint8_t* mem_set_fetch(struct vm_mem_set* mem_set, uint64_t guest_paddr) {
    for (int i = 0; i < mem_set->cnt; i++) {
        if (mem_set->mems[i].guest_start_paddr <= guest_paddr &&
            guest_paddr < mem_set->mems[i].guest_start_paddr + mem_set->mems[i].memory_size) {
