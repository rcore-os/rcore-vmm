...
static int cmos_read(struct virt_device* dev, uint16_t port, struct rvm_io_value* value) {
    printf("CMOS read handler\n");
    return 0;
}

static int cmos_write(struct virt_device* dev, uint16_t port, struct rvm_io_value* value) {
    printf("CMOS write handler\n");
    return 0;
}

static int other_read(struct virt_device* dev, uint16_t port, struct rvm_io_value* value) {
    printf("other read handler\n");
    return 1;
}

static int other_write(struct virt_device* dev, uint16_t port, struct rvm_io_value* value) {
...
static void cmos_init(struct virt_device* dev) {
    ...
    struct rvm_guest_set_trap_args trap = {
        dev->vmid, RVM_TRAP_KIND_IO, CMOS_BASE, CMOS_SIZE, (uint64_t)dev,
    };
...
static void other_init(struct virt_device* dev) {
    ...
    struct rvm_guest_set_trap_args trap = {
        dev->vmid, RVM_TRAP_KIND_IO, SYSTEM_CTRL_PORT, 1, (uint64_t)dev,
    };
    ioctl(dev->rvm_fd, RVM_GUEST_SET_TRAP, &trap);
}
