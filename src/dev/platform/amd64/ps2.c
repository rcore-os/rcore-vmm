...
static int ps2_read(struct virt_device* dev, uint16_t port, struct rvm_io_value* value) {
...
static int ps2_write(struct virt_device* dev, uint16_t port, struct rvm_io_value* value) {
...
void ps2_init(struct virt_device* dev, int rvm_fd, int vmid) {
    ...
    struct rvm_guest_set_trap_args trap = {
        dev->vmid, RVM_TRAP_KIND_IO, PS2_BASE, PS2_SIZE, (uint64_t)dev,
    };
