...
static int vga_read(struct virt_device* dev, uint16_t port, struct rvm_io_value* value) {
...
static int vga_write(struct virt_device* dev, uint16_t port, struct rvm_io_value* value) {
...
void vga_init(struct virt_device* dev, int rvm_fd, int vmid) {
    ...
    struct rvm_guest_set_trap_args trap = {
        dev->vmid, RVM_TRAP_KIND_IO, VGA_BASE, VGA_SIZE, (uint64_t)dev,
    };
